...
После «Проверить пачку» или «Проверить файл» приложение прокручивает страницу к
результату и переводит туда клавиатурный фокус. Это одинаково работает для успешного
preview, отклонённых строк, HTTP-ошибки и обрыва сети. Блок является `aria-live`
region: во время запроса он помечен `aria-busy=true`, а итог объявляется целиком.
После любой ошибки import остаётся выключенным; исправьте источник и повторите
проверку. Ответ сервера, который нельзя разобрать как JSON, не маскируется под сетевую
ошибку и получает отдельное безопасное сообщение.
...
- В режиме «Сверка сторон» можно наложить оборот на лицо: выбрать физический лист и профиль, изменить прозрачность, отдельно скрыть сторону, включить/выключить зеркальное совмещение, границы реза и номера ячеек. Синяя ячейка показывает лицо, красная — оборот; подпись `печ. N → исходная M` объясняет duplex-перестановку. Текст здесь схематичен и всегда добавляется через `textContent`, поэтому даже Advanced/raw TeX не исполняется в браузере.
...
- Сетка карточек симметрично центрируется на A4 по обеим осям. Это необходимо, чтобы после long-edge/short-edge отражения нулевые offsets действительно совпадали. Поворот 180° выполняется вокруг центра карточки и не сдвигает её рамку.
...
Экранная сверка того же физического контракта:

![Наложение лиц и оборотов](images/print-overlay.png)
