...
- [x] Этап 3: транзакционное persistence.
  ...
  - [x] Активное хранилище мигрировано в SQLite schema 10 с FK, WAL, транзакциями, одноразовым backup/import legacy JSON, профилями принтера, секциями, authoring mode, presentation/typography settings и trusted template quarantine с колонтитулами.
...
- [ ] Этап 6: секции, оформление карточек и trusted LaTeX.
  ...
  - [x] Этап 6.8: колоды жёстко разделены на safe/advanced; Advanced всегда печатает raw в sandbox, а versioned-оболочка необязательна. Safe UI содержит только верхний/нижний колонтитулы и allowlisted-оформление; schema 10/export schema 7 сохраняют тип и расширенные колонтитулы.
...
Текущая автоматизированная база: 568 проходящих тестов, 0 `xfail`, включая browser E2E; branch coverage не ниже обязательного CI-порога 98%. Chromium-сценарий проверяет раздельные safe/Advanced-колоды, динамические колонтитулы и линии, direct raw-печать, trusted-колонтитулы/оболочку, focus/scroll preflight и calibration UI. Физический прогон на реальном принтере ещё обязателен: PDF/raster-проверка и калькулятор не моделируют driver margins, feed skew и аппаратный duplex offset.
...
9. [x] Advanced-контракт расширен плейсхолдерами `{{ upper_header }}` и `{{ lower_header }}` и отдельными полями для них в UI.
10. [x] Верхний и нижний safe-колонтитулы опционально отделяются горизонтальной линией allowlisted-толщины и отступа.
11. [x] Встроенные `{{ card_number }}` и `{{ card_count }}` реализованы для динамических колонтитулов карточки; [ ] пользовательские поля колоды и карточек остаются отдельным вторым этапом после подтверждения спроса.
...
JSON export schema 7 сохраняет `authoring_mode`, section, safe render settings и trusted source/header history без approval; schema 1–6 импортируются с совместимыми defaults. SQLite schema 10 хранит тип в валидируемом JSON-поле, а trusted-колонтитулы — в той же неизменяемой версии, что и source. Импортированный trusted-код делает колоду Advanced, всегда получает `imported/quarantined` и требует локального approval только для оболочки. CSV переносит `section;front;back`, но не тип или оболочку.
...
- шаблон внутренней области колоды с точными placeholders `{{ content }}`, `{{ section }}`, `{{ card_number }}`, `{{ card_count }}`, `{{ side }}`, `{{ upper_header }}`, `{{ lower_header }}`;
...
#### ✅ FEAT-HEADER-ADV-001 — колонтитулы в Advanced-режиме

- Расширить контракт Advanced-шаблона плейсхолдерами `{{ upper_header }}` и `{{ lower_header }}`.
- Добавить в Advanced-интерфейс отдельные поля «Верхний колонтитул» и «Нижний колонтитул». Не показывать рядом настройки безопасного оформления, чтобы режимы не смешивались снова.
- Значения полей в Advanced считать доверенными LaTeX-фрагментами и пропускать через тот же sandbox/bubblewrap-контур, что и остальной Advanced-контент.
...
- Версионировать и подтверждать шаблон и оба фрагмента колонтитулов как единое состояние, чтобы подтверждённый шаблон нельзя было незаметно скомбинировать с изменённым доверенным LaTeX.
...
**Выполнено 24.08.2026.** Safe-шаблон принимает только два точных числовых плейсхолдера и после подстановки проходит обычное экранирование. HTML-превью пересчитывает значения после add/delete/reorder, PDF считает логические карточки до sheet padding. Advanced-контекст получил тот же `card_count`; поля колонтитулов, оболочка, quarantine и approval образуют одну версию. SQLite 9→10 и JSON 6→7 сохраняют новые поля, а старые данные получают пустые совместимые значения.

#### ⬜ FEAT-CARD-FIELDS-001 — пользовательские поля (второй этап, после проверки спроса)
...
- запланировано вместе с `FEAT-CARD-FIELDS-001`: round-trip пользовательских полей через клонирование и экспорт/импорт;
