...
- два независимых колонтитула могут выводиться на лицах, оборотах или обеих сторонах, сверху либо снизу, со своим выравниванием;
...
Все поля являются enum или обычным экранированным текстом. Приложение само строит локальные команды `fontsize/family/series/shape`; в форму нельзя передать пакет, команду или произвольную директиву LaTeX. Auto-fit меняет размер внутри выбранного семейства и начертания. В approved Advanced-шаблоне весь built-in слой body/колонтитулов/типографики обходится целиком, поэтому два механизма не вкладываются друг в друга.
...
Активное хранилище — SQLite schema 8: `decks`, `cards`, упорядоченная связь `deck_cards(position)`, `deck_render_settings`, `printer_profiles`, карантин `trusted_templates` и служебная `repository_meta`. Старые schema 1–7 автоматически и транзакционно обновляются без изменения колод. Schema 5→6 создаёт пустой trusted-карантин, 6→7 добавляет независимый `escaped/raw` mode обеих сторон, а 7→8 — versioned JSON-набор безопасной типографики и второго колонтитула с профилем `off` по умолчанию. Поэтому прежняя печатная вёрстка не меняется. Foreign keys включаются для каждого соединения, journal работает в WAL, а изменения выполняются write-транзакциями.
...
- versioned JSON schema 5 содержит deck/card UUID, тексты, секции, безопасные настройки, типографику и историю trusted source/provenance без approval; импорт schema 1–4 остаётся совместимым, а все импортированные шаблоны получают новый provenance `imported` и состояние `quarantined`;
...
Текущее состояние полного набора: 541 проходящий тест, 0 `xfail`, branch coverage 98,61% при обязательном пороге 98%. В него входят migrations до schema 8, JSON schema 1–4→5, физические row/sheet breaks, PDF-геометрия, безопасная типографика и два колонтитула, UI-inventory, calibration calculator и production health. Hostile-набор проверяет namespaces, отсутствие host/project mounts и сети, запрет host-записи/shell escape, timeout, лимиты и очистку. Chromium E2E проходит offline workflow, включая presentation/sections, preflight, trusted quarantine/approval, sandbox-routed PDF и calibration UI.
