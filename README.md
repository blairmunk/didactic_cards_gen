...
Локальное Flask-приложение для создания колод дидактических карточек и сборки A4 PDF через LaTeX. У карточки есть лицевая сторона (задание), оборотная сторона (ответ) и сохраняемая секция/тема; данные сохраняются транзакционно в SQLite. Поддерживаются проверяемый пакетный ввод, строгий mode-aware CSV, формулы, сортировка, клонирование и обратимое удаление колод, сохраняемые профили принтера, двухстраничный калибровочный PDF, duplex PDF, физическое наложение лиц/оборотов перед печатью и отдельные PDF сторон для ручной двусторонней печати.

> Статус после remediation 02.09.2026: duplex transforms, поворот оборота 0°/180° вокруг центра, центрированная A4-сетка, физический front/back overlay, calibration, auto-fit и адресный preflight реализованы. SQLite schema 15 и JSON export schema 8 жёстко разделяют неизменяемые Safe/Advanced типы колод; schema 15 добавляет атомарную корзину с restore и отдельным purge. Поддерживается только явная migration-цепочка 12→13→14→15 с проверенным pre-migration backup. Safe использует экранированный текст, согласованные HTML/PDF строки и абзацы, профили типографики и динамические колонтитулы; Advanced передаёт raw TeX и необязательные versioned front/back-оболочки только в sandbox. Strict CSV переносит mode-aware поля без изменения raw-значений, требует подписанный preview и переводит клавиатурный фокус к любому результату проверки, включая HTTP/network ошибку. Гарантия точности печати всё ещё требует физического прогона; актуальный прогресс — в [плане ревизии](docs/REMEDIATION_PLAN.md).
...
В редакторе переключитесь на «Сверка сторон»: приложение покажет лист A4 с
регулируемой прозрачностью оборота, границами реза и номерами печатных/исходных
ячеек. Выбранный здесь профиль автоматически применяется к формам PDF.

![Сверка лицевой и оборотной сторон](docs/images/print-overlay.png)
...
Срок хранения колод в корзине по умолчанию равен 30 дням. Deployment может
задать от 1 до 3650 дней через `DIDACTIC_CARDS_TRASH_RETENTION_DAYS`.
...
автоматически создаст pre-restore backup. При старте schema 12/13/14 база автоматически
бэкапится в один stable-файл перед цепочкой 12→13→14→15, 13→14→15 либо шагом
14→15. Шаг 13→14 очищает только ошибочно сохранённые raw-колонтитулы
Safe-карточек, оставляя исходные значения в backup, и не меняет Advanced-данные.
Шаг 14→15 добавляет nullable trash timestamps; существующие колоды остаются активными.
Перед upgrade остановите **все** workers schema 12/13/14: rolling upgrade со
