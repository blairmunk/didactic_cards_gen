...
3. CSV — текущий формат использует колонки «секция; лицевая; оборотная», прежние двухколоночные файлы «лицевая; оборотная» также поддерживаются.
...
> Важно для Advanced: текущие bulk/CSV являются legacy-инструментами и ещё не
> обеспечивают lossless raw-импорт. Bulk интерпретирует обратные слэши, CSV обрезает
> внешние пробелы, а `upper_header/lower_header` не поддерживаются и не экспортируются.
> Для переноса всей Advanced-колоды используйте JSON. Строгий пятиколоночный CSV для
> подготовки карточек во внешней программе/нейросети запланирован и подробно описан в
> [аудите bulk и CSV](BULK_CSV_IMPORT_AUDIT.md).
...
- текущий UTF-8-BOM CSV использует `;`, header `section;front;back` и стандартное quoting для delimiter/multiline; прежние строки `front;back` принимаются без секции. Он не переносит значения Advanced-колонтитулов и пока не является lossless-форматом для raw-карточек;
