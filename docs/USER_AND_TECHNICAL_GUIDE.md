...
Активное хранилище — SQLite schema 3: `decks`, `cards`, упорядоченная связь `deck_cards(position)`, `printer_profiles` и служебная `repository_meta`. Старые schema 1/2 автоматически и транзакционно обновляются без изменения колод; при переходе с schema 2 long-edge профили получают восстановленный поворот 180°, short-edge — 0°. Foreign keys включаются для каждого соединения, journal работает в WAL, а изменения колоды, карточек и профилей выполняются write-транзакциями. Поэтому конкурентные процессы не теряют добавления, а исключение внутри mutation откатывает всю операцию.
