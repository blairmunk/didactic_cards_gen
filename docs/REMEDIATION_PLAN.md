...
- [ ] Этап 6: секции, оформление карточек и trusted LaTeX.
  ...
  - [ ] Исправить обычные регистрационные метки и добавить независимый координатный raster oracle.
...
|---|---|---|---|
| **P1 `BUG-PRINT-010`** | Обычные регистрационные метки используют TikZ `remember picture` и `current page`, тогда как `PdfLatexCompiler` запускает TeX один раз. Реальная сборка сообщает, что labels могли измениться, а raster показывает один крест около верхнего центра вместо восьми меток по краям на обеих сторонах. Существующий golden был создан тем же ошибочным кодом и закрепляет дефект. | Убрать зависимость от второго прохода: привязать метки к локальному блоку страницы/сетки с явными миллиметровыми координатами, аналогично исправленному calibration sheet. Не регенерировать golden до появления независимого oracle. | Один проход TeX без rerun warning; на front и back найдены все восемь меток в заданных координатах с допуском; MediaBox A4; метки не меняют cut size и раскладку. |
| **P2 `BUG-PRINT-011`** | `printable_area_warnings()` считает любое отрицательное X/Y-смещение выходом за область, даже если сетка остаётся на листе. Положительное смещение той же величины может не дать предупреждения. Это создаёт ложные сообщения для нормальной калибровки. | Считать полный bounding box сетки после offset и сравнивать четыре его края с физической областью печати; позднее брать несимметричные поля из printer profile. | Boundary-тесты для обоих знаков, обеих осей и обеих сторон: безопасный сдвиг не предупреждается, пересечение каждого из четырёх краёв предупреждается. |
...
| **P2 `BUG-DATA-006`** | У доменной модели и SQLite schema 2 нет секции карточки и versioned render settings колоды. JSON schema 1 и CSV сохраняют только прежнее представление. Добавление полей только в форму приведёт к их потере при clone/import/export. | Выполнить schema 3 migration и schema 2 export contract до UI-функций; все изменения настроек увеличивают `deck.version`. | Старые базы мигрируются без изменения вывода, clone и JSON round-trip сохраняют всё, legacy JSON/CSV импортируется детерминированно. |
...
- позже — `section_break`: без разрыва / с новой строки сетки / с нового листа.
...
- ровно восемь меток в ожидаемых координатах front/back при одном проходе TeX;
...
1. **6.0 — геометрические багфиксы:** `BUG-PRINT-010`, затем `BUG-PRINT-011`; сначала падающие координатные тесты, потом исправление и только затем обновление golden.
2. **6.1 — данные и совместимость:** schema 3, `DeckRenderSettings`, `Card.section`, JSON schema 2, расширенный CSV, clone/snapshot/version tests. UI пока не включается.
...
Этап считается завершённым, когда миграция сохраняет прежний PDF существующей колоды, новые built-in функции проходят полную матрицу unit/integration/browser/PDF tests, а trusted mode недоступен без проверенного sandbox и явного согласия владельца. Наиболее безопасный следующий рабочий инкремент — только пункт 6.0; он исправляет уже существующее неверное обещание печатной диагностики и не требует ещё принимать продуктовые решения о шаблонах.
