...
9. Расширение Advanced-контракта плейсхолдерами `{{ upper_header }}` и `{{ lower_header }}` и отдельными полями для них в UI.
10. Опциональное отделение верхнего и нижнего колонтитулов от содержимого карточки горизонтальной линией.
11. Динамические колонтитулы карточки: сначала встроенные `{{ card_number }}` и `{{ card_count }}`, затем — при подтверждённой необходимости — пользовательские поля колоды и карточек.
...
### 7.9. План расширения колонтитулов
...
#### Обязательные тесты расширения колонтитулов
...
- integration: round-trip пользовательских полей через клонирование и экспорт/импорт, когда будет реализован второй этап;
