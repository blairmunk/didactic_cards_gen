# Исторический архив ревизии

> Архив не является актуальной спецификацией. В нём сохранены промежуточные
> планы и контракты удалённых JSON-backend, legacy CSV и SQLite schema 1–12;
> часть отмеченных там миграций и recovery-команд больше не существует.
> Действующий план: [REMEDIATION_PLAN.md](REMEDIATION_PLAN.md).
...
| ~~BUG-ARCH-001~~ ✅ | Старый `JsonFileStorage` импортировал удалённый `StorageBackend`; весь старый test collection раньше падал. | Выполнено на этапе 0; после окончательного удаления совместимости активным остаётся только `SqliteRepository`. |
...
| ~~BUG-IMP-002~~ ✅ | UI обещал `;`, `csv.reader` использовал `,`. | Выполнено и расширено: обязательный canonical header, auto/explicit comma-semicolon-tab, UTF-8/UTF-16/Windows-1251, mode-aware 3/5-column mapping, подписанный read-only preview и atomic reject. |
...
| ~~BUG-CONF-001~~ ✅ | База зависела от process CWD. | Выполнено: стабильный абсолютный `app/data`, override через `DIDACTIC_CARDS_DATA_DIR`; оба CWD дают один путь. |
...
Полный перечень подводных камней, принятые решения, security boundary и актуальная
тестовая матрица находятся в
[BULK_CSV_IMPORT_AUDIT.md](BULK_CSV_IMPORT_AUDIT.md). Этот документ является источником
истины для реализации серии `IMPORT-*`.

### 7.11. ✅ Удаление устаревших контрактов и тестовых данных

Выполнено 24.08.2026 по прямому решению владельца удалить тестовые данные и не нести
стоимость обратной совместимости:

- удалены JSON repository, его recovery/backup script paths, одноразовый import и
  тесты старого backend;
- SQLite поднят до чистой schema 12; принимается только пустая база или точная текущая
  версия, старые схемы не изменяются и не угадываются;
- удалены positional CSV без заголовка, bulk backslash-escaping и переключатели схемы
  из UI/API/use cases;
- удалены preset прежней top-left вёрстки и неиспользуемые escaped/raw selectors
  Advanced-оболочки; Advanced-контент всегда raw по определению;
- JSON export/import принимает только текущую schema 8;
- рабочий `app/data` очищен и пересоздан как пустая SQLite schema 12;
- добавлены отдельные regression-тесты comma/semicolon dialect, quoted delimiter,
  doubled quote, CRLF/multiline raw, whitespace/backslash preservation, malformed CSV
  и атомарного отказа при лишней колонке.

Настройки long-edge/short-edge и явный поворот оборота 0°/180° сохранены: это текущая
физическая модель печати, а не слой совместимости.

### 7.12. ✅ CSRF при нескольких WSGI workers

Исправлено 24.08.2026: без `DIDACTIC_CARDS_SECRET_KEY` каждый Gunicorn worker раньше
получал отдельный случайный Flask secret, поэтому GET и следующий POST могли попасть
в разные процессы и завершиться ошибкой «Недействительный CSRF-токен». Локальный
fallback теперь хранится в process-safe `app/data/.secret_key` с правами `0600`;
явный environment secret сохраняет приоритет. Regression-тест передаёт session cookie
между двумя независимыми Flask app instances и успешно создаёт колоду.

Одновременно реальный Advanced TeX integration-тест выявил, что прежний wall-clock
