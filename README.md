...
> Статус после начала remediation 22.08.2026: программный порядок duplex-страниц исправлен (`front-1, back-1, front-2, back-2`), добавлены long-edge/short-edge transforms, независимые X/Y calibration offsets и registration marks, а заданный размер теперь относится к внешней рамке карточки и проверяется по векторной геометрии PDF. Гарантия точности всё ещё требует физического прогона; скачивание PDF с кириллическим названием колоды также пока ломает реальный dev-сервер. Актуальный прогресс — в [плане ревизии](docs/REMEDIATION_PLAN.md).
