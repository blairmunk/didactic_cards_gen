...
Все исходные regression-контракты переведены из `xfail` в обычные тесты. Текущая база — 270 проходящих тестов без `xfail`, branch coverage 98.89%; отдельный Chromium E2E запускается marker-ом `browser`. GitHub Actions проверяет Python 3.11–3.13 и отдельно запускает реальную TeX-интеграцию с порогом branch coverage 98%.
