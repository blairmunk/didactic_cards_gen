...
Текущая автоматизированная база: 414 проходящих основных тестов, 0 `xfail`, один отдельно запускаемый browser E2E; общий branch coverage составляет 98,56% при обязательном CI-пороге 98%. Chromium-сценарий проверяет настройки оформления, секцию/колонтитул, focus/scroll результата preflight и только локальные resource URLs. Физический прогон на нескольких моделях принтеров ещё обязателен: PDF/raster-проверка не моделирует driver margins, feed skew и аппаратный duplex offset.
...
4. **6.3 — секционирование:** bulk section выполнен; повтор/смена колонтитула и физические row/sheet breaks остаются следующим инкрементом.
