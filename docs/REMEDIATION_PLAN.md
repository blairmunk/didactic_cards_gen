...
Текущая автоматизированная база: более 96 проходящих тестов и строгие `xfail`-контракты для подтверждённых дефектов. Поддерживаемый путь (`JsonRepository`, renderer, compilers, use cases, web) имеет около 99% line coverage; общая цифра ниже из-за сломанного неиспользуемого `json_storage.py`, который намеренно оставлен как отдельный архитектурный дефект. Физический прогон на нескольких моделях принтеров ещё обязателен: PDF-проверка не моделирует driver margins, feed skew и аппаратный duplex offset.
...
| BUG-ARCH-001 | Старый `JsonFileStorage` импортирует удалённый `StorageBackend`; весь старый test collection раньше падал. | Решить миграцию: удалить dead adapter и тесты либо восстановить интерфейс/adapter compatibility. Не держать два конкурирующих JSON-формата. |
...
1. Принять `requirements.txt`, `requirements-dev.txt`, `pytest.ini` и новый набор тестов.
2. Удалить/мигрировать dead adapters; добиться 100% collection без import errors.
3. Нормализовать file modes, не смешивая это с логическими изменениями.
4. Добавить CI на Python 3.11, 3.12, 3.13: unit всегда, TeX integration в image с фиксированной TeX Live.
5. В CI считать strict xfail техническим долгом и публиковать их перечень; каждый fix превращает соответствующий тест в обычный.
